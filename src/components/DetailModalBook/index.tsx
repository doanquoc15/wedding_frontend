  ...
  const onSubmit = async (data: any) => {
    try {
      const dataBook = {
        ...
        time:  times.filter(item => activeElements.some(id => id === item.id))?.map(el => el?.time),
        serviceId,
        comboMenuId,
    ...
    <div>
      {
        !isPayment ? ( <div className="min-w-[500px] h-auto p-6 relative">
            ...
            >
              ...
              <div className="flex justify-between mb-6">
                ...
                <div className="w-[326px]">
                  ...
                  <div>
                    {zoneCurrent == "Khu thường" && <span className="text-[11px] text-[--clr-red-400]">Giá phòng : 1.500.000 VND</span>}
                    {zoneCurrent == "Khu vip" && <span className="text-[11px] text-[--clr-red-400]">Giá phòng : 4.000.000 VND</span>}
                  </div>
                </div>
              </div>
              <div className="flex justify-between mb-6">
          ...
          <div>
            ...
            <div className="min-w-[500px] h-auto p-6 relative flex justify-center flex-col text-[--clr-gray-500] text-[14px]">
              <div className="flex"><p className="min-w-[200px]">Tổng tiền món ăn</p> {formatMoney(priceTotalDish)} VND</div>
              <div className="flex"><p className="min-w-[200px]">Tổng tiền dịch vụ</p> {formatMoney(getDecimal(service?.price))} VND</div>
              <div className="flex"><p className="min-w-[200px]">Tổng tiền khu vực</p> 
                {zoneCurrent == "Khu thường" ? formatMoney(1500000) : (zoneCurrent == "Khu vip" ? formatMoney(4000000) : 0)} VND</div>
              <div className="flex"><p className="min-w-[200px]">Tổng tiền:</p> {formatMoney(calculatorAllPrice())} VND</div>
              <div className="flex mt-5"><p className="min-w-[200px]">Tiền cọc:</p> {formatMoney(calculatorAllPrice()*15/100)} VND</div>
              <p className="text-[--clr-red-400] text-[12px]"><span>*</span>  Tiền cọc bằng 15% tổng tiền</p>
              <ButtonBtn startIcon={isCheckout && <LoadingButton/>} onClick={makePayment} sx={{ marginTop:"20px" }}>Thanh toán</ButtonBtn>
            </div>
          </div>
      }
    </div>
